class UsersController < ApplicationController
  def show
    ...
    @upcoming_events = Event.where("date >= ?", Date.current).joins(:event_attendings).where("event_attendings.event_attendee_id": current_user.id)
    @past_events = Event.where("date < ?", Date.current).joins(:event_attendings).where("event_attendings.event_attendee_id": current_user.id)
